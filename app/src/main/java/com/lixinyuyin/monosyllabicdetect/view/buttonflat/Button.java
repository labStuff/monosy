package com.lixinyuyin.monosyllabicdetect.view.buttonflat;
...
public abstract class Button extends CustomView {
    ...
    int backgroundColor = Color.parseColor("#616161");
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (isEnabled()) {
            ...
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                ...
                x = event.getX();
                y = event.getY();
                if (!((event.getX() <= getWidth() && event.getX() >= 0) && (event.getY() <= getHeight() && event
                        .getY() >= 0))) {
                    isLastTouch = false;
                    x = -1;
                    y = -1;
    ...
    public Bitmap makeCircle() {
        Bitmap output =
                Bitmap.createBitmap(getWidth() - Utils.dpToPx(6, getResources()), getHeight()
