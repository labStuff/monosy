...
public class FloatingEditText extends EditText {
	...
	public FloatingEditText(Context context, AttributeSet attrs,
		...
		mHintScale = attributes.getFloat(
				R.styleable.FloatingEditText_floating_edit_text_hint_scale,
				HINT_SCALE);
		...
		mErrorColor = attributes
				.getColor(
						R.styleable.FloatingEditText_floating_edit_text_error_color,
						getResources().getColor(
								R.color.floating_edit_text_error_color));
		...
		Drawable drawable = new Drawable() {
			...
			public void draw(Canvas canvas) {
				if (mVerified) {
					...
					float y = rect.bottom
							+ (dpToPx(16) - mHintPaint.getFontMetricsInt().top)
							/ 2;
	...
	public void setValidateResult(boolean verified, String message) {
		if (!verified && message == null) {
			throw new IllegalStateException(
					"Must have a validate result message.");
		}
	...
	protected void onDraw(@NonNull Canvas canvas) {
		...
		if (!TextUtils.isEmpty(getHint())) {
			...
			float minHintY = getBaseline() + getPaint().getFontMetricsInt().top
					+ getScrollY() - dpToPx(4);
			float textSize;
			float hintY;
			...
			case StateHintZoomIn: {
				if (elapsed < ANIMATION_DURATION) {
					textSize = ((maxTextSize - minTextSize) * elapsed)
							/ ANIMATION_DURATION + minTextSize;
					hintY = ((maxHintY - minHintY) * elapsed)
							/ ANIMATION_DURATION + minHintY;
					mHintPaint.setColor(mHighlightedColor);
					mHintPaint.setTextSize(textSize);
					canvas.drawText(getHint().toString(), hintX, hintY,
							mHintPaint);
					postInvalidate();
				} else {
					textSize = maxTextSize;
			...
			case StateHintZoomOut: {
				if (elapsed < ANIMATION_DURATION) {
					textSize = maxTextSize
							- ((maxTextSize - minTextSize) * elapsed)
							/ ANIMATION_DURATION;
					hintY = maxHintY - ((maxHintY - minHintY) * elapsed)
							/ ANIMATION_DURATION;
					mHintPaint.setColor(mHighlightedColor);
					mHintPaint.setTextSize(textSize);
					canvas.drawText(getHint().toString(), hintX, hintY,
